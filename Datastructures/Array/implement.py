...
def delete(self,index):
        for i in range(index, self.length-1):
            self.data[i] = self.data[i+1] #Shifts elements from the given index to the end by one place towards left but it does not delete the item at previous location to delete
        del self.data[self.length - 1]# this method is used
